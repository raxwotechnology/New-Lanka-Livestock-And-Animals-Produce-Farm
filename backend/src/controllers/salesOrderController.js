...
export const createSalesOrder = asyncHandler(async (req, res) => {
    ...
    // Deduct stock if order is created directly in 'approved' status (e.g. POS sales / direct checkout)
    if (order.status === 'approved') {
        const whId = order.sourceWarehouseId || warehouse?._id;
        if (whId) {
            for (const item of order.items) {
                try {
                    await decreaseStock({
                        ...
                        reason: 'Sales order created and approved',
                        userId: req.user._id,
                        allowNegative: true,
                    });
                    ...
                    console.warn(`[Sales Order Stock Deduction] Failed for ${item.productName}:`, err.message);
                }
            }
    ...
    try {
        await session.withTransaction(async () => {
            ...
            if (status === 'approved' && order.status !== 'approved') {
                for (const item of order.items) {
                    // Directly deduct onHand — stock leaves warehouse on approval (supports negative stock / backorders)
                    await decreaseStock({
                        productId: item.productId,
                        warehouseId,
                        quantity: item.orderedQuantity,
