...
export const updateBill = asyncHandler(async (req, res) => {
    ...
    Object.keys(rest).forEach(key => {
        if (rest[key] !== undefined) bill[key] = rest[key];
    });

    // Recompute totals if items changed
    if (items) {
        bill.subTotal = items.reduce((sum, item) => sum + (item.quantity * item.unitPrice), 0);
        const shipping = rest.shippingCost !== undefined ? rest.shippingCost : (bill.shippingCost || 0);
        const other = rest.otherCharges !== undefined ? rest.otherCharges : (bill.otherCharges || 0);
        bill.grandTotal = bill.subTotal + shipping + other;
        // recalculate balance due
        const paid = bill.grandTotal - bill.balanceDue; // approx amount already paid
        bill.balanceDue = bill.grandTotal - (bill.amountPaid || 0);
    }
...
export const getBills = asyncHandler(async (req, res) => {
    ...
    const {
        search, supplierId, paymentStatus, agingBucket, paymentMethod,
        startDate, endDate,
        page = 1, limit = 20,
