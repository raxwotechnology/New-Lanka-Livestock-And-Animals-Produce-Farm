...
export default function BillsPage() {
    ...
    const [filters, setFilters] = useState({
        search: '', paymentStatus: '', paymentMethod: '', agingBucket: '', page: 1, limit: 15,
    });
    ...
    let bills = data?.data || [];
    if (isDataEntry) {
        bills = bills.filter(b => isRecordRecentlyCreated(b, createdRecords) || isRecordApprovedForEdit(b, approvedEditRecords));
    }
    ...
    const columns = [
        ...
        { 
            ...
            label: 'Method', 
            render: (r) => (
                <span className={`px-2 py-1 rounded-full text-xs font-semibold ${r.paymentMethod === 'Cash' ? 'bg-green-100 text-green-800' : r.paymentMethod === 'Cheque' ? 'bg-blue-100 text-blue-800' : 'bg-orange-100 text-orange-800'}`}>
        ...
        },
        {
            ...
            render: (r) => (
                <div className="flex items-center gap-1">
                    ...
                    {(!isDataEntry || isRecordApprovedForEdit(r, approvedEditRecords) || isRecordRecentlyCreated(r, createdRecords)) && (
                        <button onClick={() => navigate(`/bills/${r._id}/edit`)}
                            title="Edit Bill"
    ...
    return (
        <div>
            <PageHeader 
                ...
                description="Track what you owe suppliers" 
                actions={
                    <Button variant="primary" onClick={() => navigate('/bills/new')}>
                        <Plus size={16} className="mr-1.5" /> New Bill
                    </Button>
                }
            />
