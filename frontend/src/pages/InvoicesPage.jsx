...
import { Plus, Search, Eye, Edit, FileText, AlertTriangle } from 'lucide-react';
...
export default function InvoicesPage() {
    ...
    const [filters, setFilters] = useState({
        search: '', paymentStatus: '', paymentMethod: '', agingBucket: '',
        page: 1, limit: 15,
    });
    ...
    if (isDataEntry) {
        invoices = invoices.filter(i => isRecordRecentlyCreated(i, createdRecords) || isRecordApprovedForEdit(i, approvedEditRecords));
    }
            ...
            render: (r) => (
                <div className="flex items-center gap-1">
                    <button onClick={(e) => { e.stopPropagation(); navigate(`/invoices/${r._id}`); }}
                        className="p-1.5 text-gray-500 hover:text-primary-600 hover:bg-primary-50 rounded" title="View Invoice">
                        <Eye size={16} />
                    </button>
