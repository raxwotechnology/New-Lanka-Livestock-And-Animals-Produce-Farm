...
import { Eye, Receipt, TrendingUp, TrendingDown, Clock, Search, Plus } from 'lucide-react';
...
export default function ChequeLedgerPage() {
    ...
    useEffect(() => {
        ...
        const backendUrl = (import.meta.env.VITE_API_URL || 'https://new-lanka-livestock-and-animals-produce.onrender.com/api').replace(/\/api\/?$/, '');
    ...
    const fmtDate = (d) => {
        ...
        return isNaN(date.getTime()) ? '—' : date.toLocaleDateString('en-LK');
    };
    ...
    const filteredPayments = payments.filter(p => {
        ...
        return (
            (p.chequeNumber && String(p.chequeNumber).toLowerCase().includes(query)) ||
            (p.bankName && String(p.bankName).toLowerCase().includes(query)) ||
            (p.partyName && String(p.partyName).toLowerCase().includes(query)) ||
            (p.paymentNumber && String(p.paymentNumber).toLowerCase().includes(query))
        );
    });
    ...
    const columns = [
        ...
        { 
            ...
            render: (r) => {
                ...
                const isPending = String(statusStr).toLowerCase() === 'pending';
                return (
                    <div 
                        onClick={(e) => {
                            if (isPending) {
    ...
    return (
        <div className="space-y-6">
            <PageHeader 
                ...
                actions={
                    <Button variant="primary" onClick={() => navigate('/payments/new?returnTo=/finance/cheques')}>
                        <Plus size={16} className="mr-1.5" /> Record Cheque Payment
                    </Button>
                } 
            />
                    ...
                    <div className="flex flex-wrap items-center gap-3">
                        ...
                        <div className="w-48">
                            <Select 
                                ...
                                onChange={(e) => setFilters((f) => ({ ...f, chequeStatus: e.target.value, page: 1 }))} 
                            />
                        </div>
                    </div>
