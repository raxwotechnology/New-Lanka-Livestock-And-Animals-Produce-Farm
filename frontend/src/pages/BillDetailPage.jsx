...
export default function BillDetailPage() {
    ...
    const canView = !isDataEntry || isRecordApprovedForEdit(bill, approvedEditRecords) || isRecordRecentlyCreated(bill, createdRecords);

    if (!canView) {
        return (
            <div className="py-16 text-center text-gray-500 max-w-md mx-auto">
                <p className="text-lg font-bold text-red-600 mb-2">Access Expired / Restricted</p>
                <p className="text-sm text-gray-600 mb-4">You do not have access to view this bill. The 5-minute approval window has expired.</p>
            ...
            <PageHeader
                ...
                actions={
                    <div className="flex gap-2">
                        ...
                        {canEdit && (
                            <Button variant="outline" onClick={() => navigate(`/bills/${bill._id}/edit`)}>
                                <Edit size={16} className="mr-1.5" /> Edit Bill
                            </Button>
                        )}
                        {bill.balanceDue > 0 && (
