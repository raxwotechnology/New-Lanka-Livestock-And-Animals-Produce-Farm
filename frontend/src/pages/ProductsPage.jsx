        ...
        },
        {
            ...
            render: (row) => (
                <div className="flex gap-1">
                    {(isAdmin || isRecordApprovedForEdit(row, useAuthStore.getState().approvedEditRecords)) && (
                        <>
                            <button
                                onClick={(e) => {
                                    e.stopPropagation();
                        ...
                        {user?.role !== 'manager' && (
                            <ExportButtons
                                ...
                                onExportAllPDF={async () => {
                                    try {
                                        const res = await api.get('/products', { params: { ...filters, page: 1, limit: 10000 } });
                                        const allData = res.data.data.map(p => ({
                                            ...p,
                                            categoryName: p.categoryId?.name || '—',
                                            brandName: p.brandId?.name || '—',
                                        }));
                                        handleExportPDF(allData);
                                    } catch (err) { alert('Failed to export all PDF'); }
                                }}
                                onExportAllExcel={async () => {
                                    try {
                                        const res = await api.get('/products', { params: { ...filters, page: 1, limit: 10000 } });
                                        const allData = res.data.data.map(p => ({
                                            ...p,
                                    ...
                                    } catch (err) { alert('Failed to export all Excel'); }
                                }}
                                onExportAllCSV={async () => {
                                    try {
                                        ...
                                        handleExportCSV(allData);
                                    } catch (err) { alert('Failed to export all CSV'); }
                                }}
